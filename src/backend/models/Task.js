// src/backend/models/Task.js
// =============================================================================
// MODELO TASK VERSION 2.0 - MINI CLASSROOM
// =============================================================================
// Características:
//   ✓ Asignación a usuarios específicos (no personas)
//   ✓ Control de quién puede completar tareas
//   ✓ Estado: pendiente, en-progreso, completada, cancelada
//   ✓ Prioridades: baja, media, alta, crítica
//   ✓ Sistema de comentarios
//   ✓ Archivos adjuntos
//   ✓ Historial de cambios
// =============================================================================

import mongoose from 'mongoose';

const DEBUG = true;
function tlog(...args) { if (DEBUG) console.log('📝 [Task]', ...args); }

// =============================================================================
// SUBDOCUMENTO: COMENTARIOS
// =============================================================================
const commentSchema = new mongoose.Schema({
  texto: {
    type: String,
    required: true,
    trim: true,
    maxlength: 1000
  },
  usuario: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true
  },
  usuarioNombre: {
    type: String,
    required: true
  },
  fecha: {
    type: Date,
    default: Date.now
  },
  editado: {
    type: Boolean,
    default: false
  },
  fechaEdicion: {
    type: Date,
    default: null
  }
}, { _id: true });

// =============================================================================
// SUBDOCUMENTO: ARCHIVOS ADJUNTOS
// =============================================================================
const attachmentSchema = new mongoose.Schema({
  nombre: {
    type: String,
    required: true
  },
  nombreOriginal: {
    type: String,
    required: true
  },
  url: {
    type: String,
    required: true
  },
  tipo: {
    type: String,
    required: true
  },
  tamaño: {
    type: Number,
    required: true
  },
  subidoPor: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true
  },
  subidoPorNombre: {
    type: String,
    required: true
  },
  fechaSubida: {
    type: Date,
    default: Date.now
  }
}, { _id: true });

// =============================================================================
// SUBDOCUMENTO: HISTORIAL DE CAMBIOS
// =============================================================================
const historySchema = new mongoose.Schema({
  campo: {
    type: String,
    required: true
  },
  valorAnterior: mongoose.Schema.Types.Mixed,
  valorNuevo: mongoose.Schema.Types.Mixed,
  modificadoPor: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true
  },
  modificadoPorNombre: {
    type: String,
    required: true
  },
  fecha: {
    type: Date,
    default: Date.now
  }
}, { _id: true });

// =============================================================================
// MODELO PRINCIPAL TASK
// =============================================================================
const taskSchema = new mongoose.Schema({
  // ===== CAMPOS BÁSICOS =====
  titulo: {
    type: String,
    required: [true, 'El título es obligatorio'],
    trim: true,
    maxlength: 200
  },
  descripcion: {
    type: String,
    default: '',
    trim: true,
    maxlength: 5000
  },
  
  // ===== ESTADO Y PRIORIDAD =====
  estado: {
    type: String,
    enum: ['pendiente', 'en-progreso', 'completada', 'cancelada'],
    default: 'pendiente'
  },
  prioridad: {
    type: String,
    enum: ['baja', 'media', 'alta', 'critica'],
    default: 'media'
  },
  
  // ===== ASIGNACIÓN (CLAVE PARA MINI CLASSROOM) =====
  tipo: {
    type: String,
    enum: ['personal', 'asignada', 'grupal', 'clase'],
    default: 'personal'
  },
  
  // Usuarios asignados a la tarea
  asignado_a: [{
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User'
  }],
  
  // Usuario que puede completar la tarea (para tareas individuales)
  completado_por: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    default: null
  },
  
  // Usuario que creó la tarea
  creado_por: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true
  },
  creado_por_nombre: {
    type: String,
    required: true
  },
  
  // ===== FECHAS =====
  fecha_limite: {
    type: Date,
    default: null
  },
  hora_limite: {
    type: String,
    default: null
  },
  fecha_completada: {
    type: Date,
    default: null
  },
  fecha_inicio: {
    type: Date,
    default: Date.now
  },
  
  // ===== CATEGORÍA Y ETIQUETAS =====
  categoria: {
    type: String,
    default: ''
  },
  etiquetas: [{
    type: String,
    trim: true
  }],
  
  // ===== NOTIFICACIONES =====
  recordatorio: {
    type: Boolean,
    default: false
  },
  recordatorio_enviado: {
    type: Boolean,
    default: false
  },
  
  // ===== COMENTARIOS Y ARCHIVOS =====
  comentarios: [commentSchema],
  archivos: [attachmentSchema],
  
  // ===== HISTORIAL =====
  historial: [historySchema],
  
  // ===== METADATOS =====
  activo: {
    type: Boolean,
    default: true
  },
  fecha_creacion: {
    type: Date,
    default: Date.now
  },
  fecha_actualizacion: {
    type: Date,
    default: Date.now
  }
}, {
  timestamps: true
});

// =============================================================================
// ÍNDICES PARA OPTIMIZACIÓN
// =============================================================================
taskSchema.index({ asignado_a: 1 });
taskSchema.index({ creado_por: 1 });
taskSchema.index({ estado: 1, prioridad: -1 });
taskSchema.index({ fecha_limite: 1 });
taskSchema.index({ tipo: 1 });

// =============================================================================
// MIDDLEWARE: ACTUALIZAR fecha_actualizacion ANTES DE GUARDAR
// =============================================================================
taskSchema.pre('save', function(next) {
  this.fecha_actualizacion = new Date();
  next();
});

// =============================================================================
// MÉTODOS DE INSTANCIA
// =============================================================================

/**
 * Verifica si un usuario tiene permiso para VER la tarea
 * @param {ObjectId} userId - ID del usuario
 * @returns {boolean}
 */
taskSchema.methods.canView = function(userId) {
  // El creador puede ver
  if (this.creado_por && this.creado_por.toString() === userId.toString()) {
    return true;
  }
  // Usuarios asignados pueden ver
  if (this.asignado_a.some(id => id.toString() === userId.toString())) {
    return true;
  }
  // Tareas personales: solo el dueño
  if (this.tipo === 'personal') {
    return false;
  }
  return false;
};

/**
 * Verifica si un usuario puede completar la tarea
 * @param {ObjectId} userId - ID del usuario
 * @returns {boolean}
 */
taskSchema.methods.canComplete = function(userId) {
  // Si ya está completada, no se puede completar de nuevo
  if (this.estado === 'completada') {
    return false;
  }
  
  // El creador puede completar
  if (this.creado_por && this.creado_por.toString() === userId.toString()) {
    return true;
  }
  
  // Para tareas asignadas individualmente, solo el asignado puede completar
  if (this.tipo === 'asignada' && this.asignado_a.length === 1) {
    return this.asignado_a.some(id => id.toString() === userId.toString());
  }
  
  // Para tareas grupales, cualquier asignado puede completar
  if (this.tipo === 'grupal' || this.tipo === 'clase') {
    return this.asignado_a.some(id => id.toString() === userId.toString());
  }
  
  // Tareas personales: solo el dueño
  if (this.tipo === 'personal') {
    return this.creado_por && this.creado_por.toString() === userId.toString();
  }
  
  return false;
};

/**
 * Verifica si un usuario puede editar la tarea
 * @param {ObjectId} userId - ID del usuario
 * @returns {boolean}
 */
taskSchema.methods.canEdit = function(userId) {
  // Solo el creador puede editar (para mantener integridad)
  return this.creado_por && this.creado_por.toString() === userId.toString();
};

/**
 * Verifica si un usuario puede eliminar la tarea
 * @param {ObjectId} userId - ID del usuario
 * @returns {boolean}
 */
taskSchema.methods.canDelete = function(userId) {
  // Solo el creador puede eliminar
  return this.creado_por && this.creado_por.toString() === userId.toString();
};

/**
 * Marca la tarea como completada
 * @param {ObjectId} userId - ID del usuario que completa
 * @param {string} userName - Nombre del usuario
 */
taskSchema.methods.complete = async function(userId, userName) {
  if (this.estado === 'completada') {
    throw new Error('La tarea ya está completada');
  }
  
  if (!this.canComplete(userId)) {
    throw new Error('No tienes permiso para completar esta tarea');
  }
  
  // Registrar en historial
  this.historial.push({
    campo: 'estado',
    valorAnterior: this.estado,
    valorNuevo: 'completada',
    modificadoPor: userId,
    modificadoPorNombre: userName,
    fecha: new Date()
  });
  
  this.estado = 'completada';
  this.completado_por = userId;
  this.fecha_completada = new Date();
  await this.save();
};

/**
 * Agrega un comentario a la tarea
 * @param {Object} commentData - Datos del comentario
 */
taskSchema.methods.addComment = async function(commentData) {
  this.comentarios.push({
    texto: commentData.texto,
    usuario: commentData.usuario,
    usuarioNombre: commentData.usuarioNombre,
    fecha: new Date()
  });
  await this.save();
};

/**
 * Agrega un archivo adjunto
 * @param {Object} fileData - Datos del archivo
 */
taskSchema.methods.addAttachment = async function(fileData) {
  this.archivos.push({
    nombre: fileData.nombre,
    nombreOriginal: fileData.nombreOriginal,
    url: fileData.url,
    tipo: fileData.tipo,
    tamaño: fileData.tamaño,
    subidoPor: fileData.subidoPor,
    subidoPorNombre: fileData.subidoPorNombre
  });
  await this.save();
};

/**
 * Serializa la tarea para respuesta JSON
 */
taskSchema.methods.toJSON = function() {
  const obj = this.toObject();
  
  // Formatear fechas para mejor visualización
  if (obj.fecha_limite) {
    obj.fecha_limite_formateada = obj.fecha_limite.toISOString().split('T')[0];
  }
  
  // Contar comentarios y archivos
  obj.comentarios_count = obj.comentarios?.length || 0;
  obj.archivos_count = obj.archivos?.length || 0;
  
  // Determinar si tiene comentarios no leídos (para futura implementación)
  obj.tiene_nuevos_comentarios = false;
  
  return obj;
};

// =============================================================================
// MÉTODOS ESTÁTICOS
// =============================================================================

/**
 * Obtiene tareas asignadas a un usuario
 * @param {ObjectId} userId - ID del usuario
 * @param {Object} filters - Filtros adicionales
 */
taskSchema.statics.getUserTasks = async function(userId, filters = {}) {
  const query = {
    activo: true,
    $or: [
      { creado_por: userId },
      { asignado_a: userId }
    ]
  };
  
  // Aplicar filtros
  if (filters.estado && filters.estado !== 'all') {
    query.estado = filters.estado;
  }
  
  if (filters.prioridad && filters.prioridad !== 'all') {
    query.prioridad = filters.prioridad;
  }
  
  if (filters.tipo && filters.tipo !== 'all') {
    query.tipo = filters.tipo;
  }
  
  if (filters.search) {
    query.$or = [
      { titulo: { $regex: filters.search, $options: 'i' } },
      { descripcion: { $regex: filters.search, $options: 'i' } }
    ];
  }
  
  return this.find(query)
    .sort({ fecha_limite: 1, prioridad: -1 })
    .populate('asignado_a', 'usuario correo')
    .populate('creado_por', 'usuario correo')
    .lean();
};

/**
 * Obtiene estadísticas de tareas para un usuario
 * @param {ObjectId} userId - ID del usuario
 */
taskSchema.statics.getUserStats = async function(userId) {
  const query = {
    activo: true,
    $or: [
      { creado_por: userId },
      { asignado_a: userId }
    ]
  };
  
  const [total, pendientes, enProgreso, completadas, vencidas] = await Promise.all([
    this.countDocuments(query),
    this.countDocuments({ ...query, estado: 'pendiente' }),
    this.countDocuments({ ...query, estado: 'en-progreso' }),
    this.countDocuments({ ...query, estado: 'completada' }),
    this.countDocuments({
      ...query,
      estado: { $ne: 'completada' },
      fecha_limite: { $lt: new Date() }
    })
  ]);
  
  return { total, pendientes, enProgreso, completadas, vencidas };
};

const Task = mongoose.model('Task', taskSchema);

export default Task;